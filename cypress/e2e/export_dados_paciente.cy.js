    ...
    Cypress.Commands.add('verificarExcel', (filePath) => {
        // Espera de até 15 segundos para garantir que o arquivo seja gerado
        cy.wait(15000); 
      
        // Verifica se o arquivo existe e continua com a verificação do conteúdo
      ...
      function esperarArquivoExistir(filePath, timeout = 20000) {
        return new Cypress.Promise((resolve, reject) => {
          const interval = setInterval(() => {
            if (Cypress.fs.existsSync(filePath)) {
              clearInterval(interval);
              resolve(true);
            }
          }, 500); // Intervalo de verificação a cada 500ms
      
          setTimeout(() => {
            clearInterval(interval);
            reject(new Error(`Arquivo não encontrado no caminho: ${filePath}`));
          }, timeout);
        });
      }
    ...
    describe('Fluxo Completo de Paciente: Registro e Exportação de Dados', () => {
        ...
        it('Cenário Completo: Adicionar Dados e Exportar para Excel', () => {
            ...
            esperarArquivoExistir(filePath).then(() => {
